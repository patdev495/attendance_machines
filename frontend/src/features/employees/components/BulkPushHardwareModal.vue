          ...
          <div v-if="isOpen" class="modal-content glass shadow-2xl">
            <div class="modal-header">
              <div class="header-left">
                <div class="icon-pulse bg-emerald-500/20">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                </div>
                <div class="title-stack">
                  <h2 class="title-main">{{ $t('employees.bulk_hardware_push.title') }}</h2>
                  <p class="subtitle">{{ $t('employees.bulk_hardware_push.subtitle') }}</p>
                </div>
              </div>
            ...
            <div class="modal-body custom-scrollbar">
              ...
              <div class="section-card">
                <label class="section-title">
                  ...
                  {{ $t('employees.bulk_hardware_push.upload_label') }}
                </label>
                <div class="file-drop-zone" :class="{ 'has-file': selectedFile }">
                  ...
                  <div class="drop-zone-content" @click="$refs.fileInput.click()">
                    ...
                    <div v-else class="placeholder">
                      <span>{{ $t('employees.bulk_hardware_push.file_help') }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="section-card mt-6">
                <label class="section-title">
                  ...
                  {{ $t('employees.bulk_hardware_push.select_machines') }}
                </label>
              ...
              <div v-if="isLoading" class="progress-section mt-6">
                <div class="progress-header">
                  <span class="progress-label">{{ $t('employees.bulk_hardware_push.status_running', { current: progressInfo?.processed_count || 0, total: progressInfo?.total_machines || 0 }) }}</span>
                ...
                <div class="progress-footer">
                  <span class="current-ip" v-if="progressInfo?.current_ip">{{ $t('employees.bulk_push_progress_detail', { current: progressInfo.processed_count, total: progressInfo.total_machines, ip: progressInfo.current_ip }) }}</span>
                </div>
              </div>

              <!-- Missing/Warning Section -->
              <div v-if="missingEmployees.length > 0" class="warning-section mt-6">
                <div class="warning-header">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                  <span>{{ $t('employees.bulk_hardware_push.missing_warning', { count: missingEmployees.length }) }}</span>
                </div>
                <div class="warning-body custom-scrollbar">
                  <div class="missing-list">
                    <span v-for="id in missingEmployees" :key="id" class="missing-item">{{ id }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="results" class="results-section mt-6">
                ...
                <table class="results-table">
                  <thead>
                    <tr>
                      <th>{{ $t('employees.bulk_hardware_push.results_table.ip') }}</th>
                      <th class="text-center">{{ $t('employees.bulk_hardware_push.results_table.pushed') }}</th>
                      <th class="text-right">{{ $t('employees.bulk_hardware_push.results_table.status') }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(res, ip) in results" :key="ip">
                      <td>{{ ip }}</td>
                      <td class="text-center">{{ res.pushed }}</td>
                      <td class="text-right">
            ...
            <div class="modal-footer">
              <button class="btn-start" :disabled="isLoading || !selectedFile || selectedMachineIps.length === 0" @click="handleStart">
                ...
                {{ isLoading ? $t('employees.bulk_hardware_push.status_running', { current: '...' }) : $t('employees.bulk_hardware_push.start') }}
              </button>
            </div>
          </div>
        </Transition>
...
async function handleStart() {
  ...
  progressInfo.value = { processed_count: 0, total_machines: selectedMachineIps.value.length, current_ip: t('employees.bulk_hardware_push.initializing') }
  
  try {
    await employeesApi.bulkPushHardware(selectedFile.value, selectedMachineIps.value)
    startPolling()
  } catch (e) {
    console.error('Bulk hardware push failed', e)
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    try {
      const status = await employeesApi.getBulkPushStatus()
      progressInfo.value = status
      if (status.missing_employees) {
        missingEmployees.value = status.missing_employees
      }
