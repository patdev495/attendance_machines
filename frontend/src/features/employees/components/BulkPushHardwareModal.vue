          ...
          <div v-if="show" class="modal-content glass shadow-2xl">
            <div class="modal-header">
              <div class="header-left">
                ...
                <div class="title-stack">
                  <h2 class="title-main">{{ $t('employees.bulk_push.title') }}</h2>
                  <p class="subtitle">{{ $t('employees.bulk_push.subtitle') }}</p>
                </div>
              </div>
            ...
            <div class="modal-body custom-scrollbar">
                ...
                <div class="file-drop-zone" :class="{ 'has-file': employeeIds.length > 0 }">
                  ...
                  <div class="drop-zone-content" @click="!isProcessing && $refs.fileInput.click()">
                    <div v-if="employeeIds.length > 0" class="file-info">
                      <span class="file-name" style="color: #10b981;">{{ $t('employees.bulk_push.drop_zone_active', { count: employeeIds.length }) }}</span>
                    </div>
                    <div v-else class="placeholder">
                      <span>{{ $t('employees.bulk_push.drop_zone_placeholder') }}<br><small>{{ $t('employees.bulk_push.drop_zone_help') }}</small></span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="section-card mt-6">
                <label class="section-title">
                  ...
                  {{ $t('employees.bulk_push.step_2') }}
                </label>
                <div class="machine-grid">
                  <div v-if="machines.length === 0" class="text-sm text-gray-500 italic">{{ $t('employees.bulk_push.no_machines') }}</div>
              ...
              <div class="section-card mt-6" v-if="employeeIds.length > 0 && selectedMachines.length > 0 && !isProcessing && machineProgress.total === 0">
                ...
                >
                  ...
                  {{ isPreviewing ? $t('employees.bulk_push.verifying') : $t('employees.bulk_push.step_2_5') }}
                </button>
                ...
                <div v-if="previewResult" class="results-table mt-2" style="background: rgba(255,255,255,0.02); padding: 1rem; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05);">
                  <div style="display: flex; flex-direction: column; gap: 0.5rem; font-size: 0.85rem; color: #e2e8f0;">
                    <div style="display: flex; justify-content: space-between;">
                      <span style="color: #94a3b8;">{{ $t('employees.bulk_push.total_ids') }}</span>
                      <strong style="color: #fff;">{{ previewResult.total_input_ids }}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                      <span style="color: #94a3b8;">{{ $t('employees.bulk_push.valid_employees') }}</span>
                      <strong style="color: #10b981;">{{ previewResult.employees_found_in_db }}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                      <span style="color: #94a3b8;">{{ $t('employees.bulk_push.employees_with_fp') }}</span>
                    ...
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 0.5rem; margin-top: 0.25rem;">
                      <span style="color: #94a3b8; font-weight: bold;">{{ $t('employees.bulk_push.total_fp_to_push') }}</span>
              ...
              <div v-if="isProcessing || machineProgress.total > 0" class="progress-section mt-6">
                <div class="progress-header">
                  <span class="progress-label">{{ $t('employees.bulk_push.overall_progress', { processed: empProgress.processed, total: empProgress.total }) }}</span>
                ...
                <div class="progress-footer" style="margin-top: 0.75rem; display: flex; flex-direction: column; gap: 0.3rem;">
                  <span style="color: #94a3b8; font-size: 0.8rem;">{{ $t('employees.bulk_push.machines_completed', { processed: machineProgress.processed, total: machineProgress.total }) }}</span>
                  <span v-if="activeIps.length > 0" style="color: #818cf8; font-size: 0.8rem;">{{ $t('employees.bulk_push.processing', { ips: activeIps.join(', ') }) }}</span>
                </div>
              </div>
              ...
              <div v-if="Object.keys(results).length > 0" class="results-section mt-6">
                ...
                <table class="results-table">
                  <thead>
                    <tr>
                      <th>{{ $t('employees.bulk_push.table_ip') }}</th>
                      <th class="text-right">{{ $t('employees.bulk_push.table_status') }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(msg, ip) in results" :key="ip">
                      <td>{{ ip }}</td>
                      <td class="text-right">
                        <span class="status-badge" :class="msg.includes('Lỗi') || msg.includes('Error') ? 'error' : 'success'">
                          {{ msg }}
                        </span>
                      </td>
                    </tr>
            ...
            <div class="modal-footer">
              ...
              >
                ...
                {{ isProcessing ? $t('employees.bulk_push.btn_processing') : (isComplete ? $t('employees.bulk_push.btn_close') : $t('employees.bulk_push.btn_confirm')) }}
              </button>
            </div>
          </div>
        </Transition>
  ...
  try {
    ...
    if (!status.is_running) {
      if (status.total_machines > 0) {
        ...
        emit('success')
        notify?.success(t('employees.bulk_push.complete_notify'))
      } else {
