<template>
  <div class="machine-card card" :class="{ clickable: machine.status === 'Online' }" @click="onClick">
    <div class="card-header">
      <div class="ip">{{ machine.ip }}</div>
      <span class="status-dot" :class="machine.status === 'Online' ? 'online' : 'offline'">
        {{ machine.status === 'Online' ? $t('common.online') : $t('common.offline') }}
      </span>
    </div>
    ...
    <div v-if="machine.status === 'Online'" class="capacity-info">
      ...
      <div class="cap-row">
        ...
        <span class="val">{{ machine.users }} / {{ machine.users_cap }}</span>
      </div>
      ...
      <div class="cap-row mt-12">
        ...
        <span class="val">{{ machine.records?.toLocaleString() }} / {{ machine.records_cap?.toLocaleString() }}</span>
      </div>
      <div class="capacity-bar">
        <div class="capacity-fill" :style="{ width: getPct(machine.records, machine.records_cap) + '%', background: fillColor(getPct(machine.records, machine.records_cap)) }"></div>
      </div>
    </div>

    <div v-else-if="machine.status === 'Offline'" class="offline-label">
    ...
    <div v-if="machine.status === 'Online'" class="hint">{{ $t('device.help_short') }}</div>
  </div>
</template>
...
<style scoped>
.machine-card { padding: 20px 24px; transition: transform 0.2s, box-shadow 0.2s; min-height: 220px; display: flex; flex-direction: column; }
.machine-card.clickable { cursor: pointer; }
.machine-card.clickable:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(0,0,0,0.3); border-color: var(--primary); }
