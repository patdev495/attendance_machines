      ...
      <Transition name="scale-modal" appear>
        <div class="modal-card" @click.stop>
          <div class="modal-header">
            ...
            <div>
              <h3>{{ $t('device.sync_modal.title') }}</h3>
              <p class="subtext">{{ $t('device.sync_modal.subtitle', { id: employeeId }) }} <code>{{ sourceIp }}</code></p>
            </div>
          ...
          <div class="modal-body">
            <div class="info-banner">
              ...
              {{ $t('device.sync_modal.description') }}
            </div>

            <div v-if="loadingMachines" class="loading-state">
              <div class="spinner"></div>
              <span>{{ $t('device.sync_modal.loading_machines') }}</span>
            </div>
            ...
            <template v-else>
              <div class="list-toolbar">
                <label class="check-all">
                  ...
                  {{ $t('device.sync_modal.select_all') }}
                </label>
                <span class="selected-count">{{ $t('device.sync_modal.selected_count', { selected: selectedIps.length, total: targetMachines.length }) }}</span>
              </div>
              ...
              <div class="machines-list">
                ...
                <div v-if="targetMachines.length === 0" class="no-machines">
                  {{ $t('device.sync_modal.no_other_machines') }}
                </div>
              </div>
              ...
              <div v-if="isRunning" class="progress-section">
                ...
                <span class="progress-label">
                  {{ $t('device.sync_modal.processing', { processed: syncStatus.processed_count, total: syncStatus.total_machines }) }}
                </span>
              </div>

              <div v-if="isDone" class="done-summary">
                <span class="done-ok">✓ {{ $t('device.sync_modal.success_count', { count: successCount }) }}</span>
                <span v-if="errorCount > 0" class="done-err">× {{ $t('device.sync_modal.error_count', { count: errorCount }) }}</span>
              </div>
            </template>
          </div>

          <div class="modal-actions">
            <button class="btn btn-ghost" @click="close" :disabled="isRunning">{{ $t('common.cancel') }}</button>
            <button class="btn btn-primary btn-sync" @click="startSync" :disabled="selectedIps.length === 0 || isRunning || isDone">
              <template v-if="isRunning">
                <div class="spinner-sm"></div>
                {{ $t('device.sync_modal.syncing') }}
              </template>
              <template v-else-if="isDone">
                {{ $t('device.sync_modal.done') }}
              </template>
              <template v-else>
                ...
                {{ $t('device.sync_modal.sync_to_count', { count: selectedIps.length }) }}
              </template>
            </button>
          </div>
        </div>
...
<style scoped>
.modal-backdrop { position: fixed; inset: 0; background: rgba(2, 6, 23, 0.75); backdrop-filter: blur(8px); z-index: 10010; display: flex; align-items: center; justify-content: center; padding: 20px; }
.modal-card { background: linear-gradient(145deg, #1a1f2e, #141820); border: 1px solid rgba(99, 102, 241, 0.25); border-radius: 16px; width: 100%; max-width: 500px; box-shadow: 0 24px 48px rgba(0, 0, 0, 0.5); overflow: hidden; }
.modal-header { display: flex; align-items: flex-start; gap: 14px; padding: 20px 22px 16px; border-bottom: 1px solid rgba(255,255,255,0.07); }
.header-icon { width: 44px; height: 44px; border-radius: 10px; background: rgba(99, 102, 241, 0.15); border: 1px solid rgba(99, 102, 241, 0.3); display: flex; align-items: center; justify-content: center; color: #818cf8; flex-shrink: 0; }
...
.close-btn { margin-left: auto; background: none; border: none; color: #475569; font-size: 18px; cursor: pointer; padding: 4px; line-height: 1; transition: color 0.2s; }
.close-btn:hover:not(:disabled) { color: #94a3b8; }
.close-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.modal-body { padding: 20px 22px; }
.info-banner { display: flex; align-items: flex-start; gap: 10px; background: rgba(59, 130, 246, 0.08); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 8px; padding: 10px 14px; font-size: 0.83rem; color: #93c5fd; margin-bottom: 18px; line-height: 1.5; }
.info-banner svg { flex-shrink: 0; margin-top: 2px; }
.loading-state { display: flex; align-items: center; justify-content: center; gap: 12px; padding: 32px; color: #64748b; font-size: 0.9rem; }
...
.machines-list { border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; overflow: hidden; background: rgba(0, 0, 0, 0.2); max-height: 280px; overflow-y: auto; }
.machine-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-bottom: 1px solid rgba(255,255,255,0.04); transition: background 0.15s; }
...
.check-row { display: flex; align-items: center; gap: 10px; cursor: pointer; flex: 1; }
.check-row.disabled { cursor: not-allowed; opacity: 0.6; }
.check-all { display: flex; align-items: center; gap: 8px; cursor: pointer; font-size: 0.88rem; color: #94a3b8; }
.check-row input[type="checkbox"], .check-all input[type="checkbox"] { display: none; }
.checkmark { width: 17px; height: 17px; border-radius: 4px; border: 1.5px solid #334155; background: #0f172a; position: relative; flex-shrink: 0; transition: all 0.15s; }
.check-row:hover .checkmark, .check-all:hover .checkmark { border-color: #6366f1; }
.check-row input:checked ~ .checkmark, .check-all input:checked ~ .checkmark { background: #6366f1; border-color: #6366f1; }
...
.check-row input:checked ~ .checkmark::after, .check-all input:checked ~ .checkmark::after { display: block; }
...
.no-machines { padding: 24px; text-align: center; color: #475569; font-size: 0.88rem; }
...
.progress-fill { height: 100%; background: linear-gradient(90deg, #6366f1, #818cf8); border-radius: 3px; transition: width 0.4s ease; }
.progress-label { font-size: 0.8rem; color: #64748b; }
.done-summary { display: flex; gap: 14px; margin-top: 14px; padding: 10px 14px; background: rgba(0,0,0,0.2); border-radius: 8px; font-size: 0.85rem; font-weight: 600; }
.done-ok { color: #4ade80; }
.done-err { color: #f87171; }
.modal-actions { display: flex; justify-content: flex-end; gap: 10px; padding: 16px 22px; border-top: 1px solid rgba(255,255,255,0.07); }
.btn-sync { display: flex; align-items: center; gap: 7px; background: linear-gradient(135deg, #6366f1, #4f46e5); border: none; color: white; padding: 9px 20px; border-radius: 8px; font-size: 0.88rem; font-weight: 600; cursor: pointer; transition: opacity 0.2s, transform 0.1s; }
...
.spinner-sm { width: 15px; height: 15px; border: 2px solid rgba(255,255,255,0.2); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; }
.spinner-tiny { width: 14px; height: 14px; border: 2px solid rgba(99,102,241,0.15); border-top-color: #818cf8; border-radius: 50%; animation: spin 0.7s linear infinite; margin-left: auto; }
