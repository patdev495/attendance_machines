  ...
  <section class="filters card">
    ...
    <div class="filter-row">
      ...
      <div class="filter-group" style="justify-content: flex-end; align-items: flex-end;">
        <button class="btn btn-danger" @click="store.resetFilters()">Clear Filters</button>
      </div>
    </div>
    ...
    <div class="filter-row summary-row" v-if="store.currentView === 'summary'">
      ...
      <div class="filter-group">
        ...
        <select v-model="store.summaryFilters.shift" @change="store.loadData(1)">
          ...
          <option value="N">Day Shift (N)</option>
          <option value="D">Night Shift (D)</option>
          <option value="NA">NA (Unknown)</option>
        </select>
      </div>
      <div class="filter-group" style="flex-direction: row; align-items: center; gap: 10px; padding-top: 22px;">
        ...
        <label for="onlyMissing" style="margin-bottom:0; cursor:pointer; text-transform:none; font-size:0.9rem;">Only Missing</label>
      </div>
    </div>
  </section>
</template>
