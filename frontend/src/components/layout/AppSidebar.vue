<template>
  <aside :class="['app-sidebar', ui.sidebarOpen ? 'open' : 'collapsed']">
    ...
    <nav class="sidebar-nav">
      ...
      <a 
        href="#" 
        :class="['nav-item', isActive('raw') ? 'active' : '']"
        @click.prevent="goTo('raw')"
      >
        <div class="icon">
      ...
      <a 
        href="#" 
        :class="['nav-item', isActive('summary') ? 'active' : '']"
        @click.prevent="goTo('summary')"
      >
        <div class="icon">
      ...
      <router-link 
        to="/devices" 
        :class="['nav-item', route.path.startsWith('/devices') ? 'active' : '']"
      >
        <div class="icon">
...
function isActive(view) {
  return route.path === '/' && attendance.currentView === view
}

function goTo(view) {
  const isHome = route.path === '/'
  attendance.setView(view, isHome) // Only load if already home
  if (!isHome) {
    router.push('/')
  }
}
