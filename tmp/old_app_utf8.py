...
from db import get_db, AttendanceLog, EmployeeMetadata, SessionLocal
import threading
...
# ΓöÇΓöÇΓöÇ Shift Rule Helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
XUONG1_DEPT_KEYWORD = "X╞░ß╗ƒng 1"
...
def get_shift_rules(department, shift):
    ...
    if is_xuong1:
        if shift == 'D':  # Night: 20:00 ΓåÆ 08:00 next day
...
def get_date_range(db: Session = Depends(get_db)):
    ...
    return {
        "min_date": str(result.min_dt.date()) if result.min_dt else None,
        "max_date": str(result.max_dt.date()) if result.max_dt else None,
    }
...
def run_export_task(start_date: date, end_date: date, view_mode: str):
    ...
    try:
        ...
        headers = ["M├ú nh├ón vi├¬n", "T├¬n nh├ón vi├¬n", "Ph├▓ng ban", "Nh├│m", "Ng├áy v├áo l├ám", "Ca l├ám viß╗çc", "Ghi ch├║"]
        ...
        for idx, emp_id in enumerate(sorted_emp_ids):
            ...
            with export_lock:
                export_status["progress"] = int((idx / len(sorted_emp_ids)) * 50) # First 50% for sheet 1
            ...
            for r_offset in range(num_rows):
                ...
                if view_mode == "time": all_indicators = ["Giß╗¥ In", "Giß╗¥ Out", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
                elif view_mode == "hours": all_indicators = ["Giß╗¥ C├┤ng", "T─âng Ca", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
                else: all_indicators = ["Giß╗¥ In", "Giß╗¥ Out", "Giß╗¥ C├┤ng", "T─âng Ca", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
        ...
        ws2 = wb.create_sheet(title="Th├┤ng tin chi tiß║┐t")
        ws2.append(["M├ú NV", "T├¬n nh├ón vi├¬n", "Ph├▓ng ban", "Nh├│m", "Ca", "Ng├áy", "Giß╗¥ In", "Giß╗¥ Out", "Giß╗¥ C├┤ng", "T─âng Ca", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"])
        ...
        for idx, emp_id in enumerate(sorted_emp_ids):
            ...
            for d in dates_list:
                ...
                # Ensure ot is numeric for display or 0
...
@app.post("/api/export-attendance/start")
def start_export(
...
@app.get("/api/export-attendance/status")
def get_export_status():
    with export_lock:
        return export_status

@app.post("/api/export-attendance/cancel")
def cancel_export():
    with export_lock:
        export_status["cancel_requested"] = True
    return {"message": "Cancellation requested"}

@app.get("/api/export-attendance/download")
def download_export():
    with export_lock:
...
def legacy_export_attendance(
    ...
    # Data Rows
    current_row = 2
    # time ΓåÆ 4 rows (In/Out + ─Éi muß╗Ön/Vß╗ü sß╗¢m)
    # hours ΓåÆ 4 rows (C├┤ng/T─ângCa + ─Éi muß╗Ön/Vß╗ü sß╗¢m)
    # both  ΓåÆ 6 rows
    if view_mode == "both":
        num_rows = 6
    else:
    ...
    for emp_id in sorted_emp_ids:
        ...
        for r_idx in range(current_row, current_row+num_rows):
            ...
            # Indicator column
            if view_mode == "time":
                all_indicators = ["Giß╗¥ In", "Giß╗¥ Out", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
            elif view_mode == "hours":
                all_indicators = ["Giß╗¥ C├┤ng", "T─âng Ca", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
            else:
                all_indicators = ["Giß╗¥ In", "Giß╗¥ Out", "Giß╗¥ C├┤ng", "T─âng Ca", "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"]
    ...
    # ΓöÇΓöÇΓöÇ Sheet 2: Flat/Filterable "Late & Early Leave" table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ws2 = wb.create_sheet(title="Th├┤ng tin chi tiß║┐t")

    flat_headers = [
        "M├ú NV", "T├¬n nh├ón vi├¬n", "Ph├▓ng ban", "Nh├│m", "Ca",
        "Ng├áy", "Giß╗¥ In", "Giß╗¥ Out", "Giß╗¥ C├┤ng", "T─âng Ca",
        "─Éi muß╗Ön (ph├║t)", "Vß╗ü sß╗¢m (ph├║t)"
    ]
...
# Serve Vue 3 SPA ΓÇö all non-API routes return index.html (for Vue Router history mode)
