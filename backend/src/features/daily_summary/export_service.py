...
from database import AttendanceLog, EmployeeLocalRegistry, ShiftRule, SessionLocal
...
def run_export_task(start_date: date, end_date: date, view_mode: str):
    ...
    try:
        ...
        # Using work_date calculation logic consistent with attendance router
        ...
        ).outerjoin(EmployeeLocalRegistry, AttendanceLog.employee_id == EmployeeLocalRegistry.employee_id).subquery()
        ...
        emp_meta_query = db.query(EmployeeLocalRegistry).filter(EmployeeLocalRegistry.employee_id.in_(all_emp_ids)).all()
        ...
        for idx, emp_id in enumerate(sorted_emp_ids):
            ...
            for d in dates_list:
                ...
                row_data = [
                    ...
                    emp_m.group_name if emp_m else "-", ds["shift"] or "N", d.strftime("%d/%m/%Y"), 
